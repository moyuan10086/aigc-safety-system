  ...
  <section class="evaluation-panel card" aria-live="polite">
    ...
    <template v-else>
      ...
      <div class="evidence-summary">
        ...
        <span><b>{{ summary.evaluated || 0 }}</b><small>当前发布能力</small></span>
...
const pendingTasks = computed(() => (status.tasks || []).filter((task:Record<string,any>) => ['pending_access','insufficient_samples','inconclusive','unlabeled'].includes(task.status)))

const labels: Record<string,string> = {
  'content_safety:multiheaded_q16':'图片安全 · MultiHeaded+Q16 高速初筛（UnsafeBench）',
  'content_safety:perspectivevision':'图片安全 · PerspectiveVision 高召回复核（UnsafeBench）',
  ...
  'authenticity:generic_aigc':'图片真实性 · 通用 AI 生图检测',
}
const taskLabel = (task:string) => labels[task] || task
const taskStatusLabel = (task:Record<string,any>) => ({
  ready:'统计完成', insufficient_samples:'样本不足', unlabeled:'缺少真值', archived_unlabeled:'历史盲测归档', pending_access:'待获取授权', inconclusive:'无法确认', unknown:'仅链路验证',
