    ...
    <div v-if="items.length" class="review-list">
      <article v-for="item in items" :key="item.event_id" class="review-row">
        <div class="event-meta">
          ...
          <button type="button" :class="{ reviewed: item.evidence_reviewed }" :title="item.evidence_reviewed ? '已查看证据，可再次打开核对' : '查看审计事件与加密证据'" @click="$emit('inspect', item.event_id)">
            <component :is="item.evidence_reviewed ? BadgeCheck : FileSearch" :size="15" /><span>{{ item.evidence_reviewed ? '已看证据' : '取证' }}</span>
          </button>
        </div>
        ...
        <div class="review-control" role="group" :aria-label="`${item.event_id} 人工复核标签`">
          <button v-for="choice in choices" :key="choice.value" type="button" :class="[choice.value, { active: item.review_label === choice.value }]" :disabled="busyEventId === item.event_id || !item.evidence_reviewed" :title="item.evidence_reviewed ? choice.title : '先打开取证并查看原始证据'" @click="$emit('resolve', item.event_id, choice.value)">
...
import { ArrowRight, BadgeCheck, CircleAlert, Download, EyeOff, FileSearch, GitCompareArrows, ShieldCheck, ShieldX } from 'lucide-vue-next'
