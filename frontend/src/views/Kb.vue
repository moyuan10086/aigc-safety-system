      ...
      <aside class="qa-panel">
        ...
        <div ref="chatBox" class="chat-box">
          ...
          <div v-for="(message, index) in messages" :key="index" :class="['message', message.role]">
            ...
            <span v-else class="answer-pending">{{ asking ? '正在检索证据并组织回答…' : '' }}</span>
...
// Model responses are untrusted. Markdown is supported, but embedded raw HTML is not.
markdownRenderer.html = () => ''

function renderMarkdown(content:string) {
  return marked.parse(content, { gfm:true, breaks:true, renderer:markdownRenderer }) as string
}
  ...
  try {
    ...
    while (true) {
      ...
      for (const event of events) {
        ...
        try { messages.value[index].content += JSON.parse(data) }
        catch { messages.value[index].content += data }
      }
...
/* Readable long-form Q&A workspace. These overrides intentionally keep the existing card language. */
.workspace-grid{grid-template-columns:minmax(0,1.18fr) minmax(440px,.92fr)}
.qa-panel{min-height:680px}
.chat-box{height:min(58vh,640px);min-height:480px;gap:12px;padding:16px}
.qa-empty{width:min(100%,440px);margin:auto;padding:26px 10px}
.qa-empty>strong{font-size:14px}.qa-empty p{font-size:11px;line-height:1.65}.qa-empty button{padding:10px 11px;font-size:11px}
.message{width:fit-content;max-width:94%;padding:12px 14px;font-size:13px;line-height:1.75;white-space:normal;overflow-wrap:anywhere}
.message.user{max-width:82%;white-space:pre-wrap}
.answer-pending{color:var(--faint)}
.markdown-body{min-width:0;color:var(--text)}
.markdown-body>:first-child{margin-top:0}.markdown-body>:last-child{margin-bottom:0}
.markdown-body p{margin:0 0 10px}.markdown-body h1,.markdown-body h2,.markdown-body h3{margin:18px 0 9px;color:var(--text);line-height:1.35}.markdown-body h1{font-size:18px}.markdown-body h2{font-size:16px}.markdown-body h3{font-size:14px}
.markdown-body ul,.markdown-body ol{margin:8px 0 11px;padding-left:22px}.markdown-body li+li{margin-top:4px}.markdown-body blockquote{margin:11px 0;padding:8px 11px;color:var(--muted);background:var(--surface-2);border-left:3px solid var(--primary)}
.markdown-body code{padding:2px 5px;color:#9a4913;background:#fff5ec;border-radius:3px;font:12px ui-monospace,SFMono-Regular,Consolas,monospace}.markdown-body pre{max-width:100%;margin:11px 0;padding:12px;overflow:auto;color:#243b4b;background:#f3f7f9;border:1px solid var(--line);border-radius:5px}.markdown-body pre code{padding:0;color:inherit;background:transparent;white-space:pre}
.markdown-body a{color:var(--primary);font-weight:650;text-decoration:underline;text-underline-offset:2px}.markdown-body hr{height:1px;margin:15px 0;background:var(--line);border:0}.markdown-body table{display:block;max-width:100%;margin:11px 0;overflow:auto;border-collapse:collapse;font-size:12px}.markdown-body th,.markdown-body td{min-width:96px;padding:7px 9px;text-align:left;border:1px solid var(--line)}.markdown-body th{color:var(--text);background:var(--surface-2)}
@media(max-width:1150px){.workspace-grid{grid-template-columns:minmax(0,1fr) 430px}.chat-box{min-height:440px}}
@media(max-width:880px){.workspace-grid{grid-template-columns:1fr}.qa-panel{position:static;min-height:0}.chat-box{height:min(62vh,580px);min-height:420px}}
@media(max-width:620px){.chat-box{height:56vh;min-height:420px;padding:13px}.message{max-width:100%;font-size:12px}.message.user{max-width:90%}.markdown-body h1{font-size:16px}.markdown-body h2{font-size:15px}}
</style>
