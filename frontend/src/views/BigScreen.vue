    ...
    <template v-else>
      ...
      <main class="screen-grid">
        <div class="screen-column left-column">
          <section class="screen-panel intro-panel">
            <PanelTitle title="平台简介" code="PLATFORM PROFILE" />
            <div class="platform-intro">
              <p>平台采用“真实性鉴别 + 内容安全审核”双主线，同时识别 AIGC 伪造、敏感内容与安全风险，并统一编排来源取证和人工复核。</p>
              <div class="capability-tags"><span>Deepfake</span><span>内容安全</span><span>大模型护栏</span><span>C2PA</span></div>
          ...
          <section class="screen-panel trend"><PanelTitle title="审核与告警趋势" code="EVENT TREND" /><BaseChart :option="trendOption" aria-label="大屏审核与告警趋势图" dark /></section>
          <section class="screen-panel ring"><PanelTitle title="风险类别分布" code="RISK PROFILE" /><BaseChart :option="riskOption" aria-label="大屏风险类别环形图" dark /></section>
        </div>

        <section class="screen-panel situation">
          <PanelTitle title="多模态审核防御链" code="DEFENSE PIPELINE" />
        ...
        <div class="screen-column right-column">
          <section class="screen-panel alerts">
            <PanelTitle title="实时风险事件" code="LIVE ALERTS" />
          ...
          <section class="screen-panel engines"><PanelTitle title="模型与引擎" code="ENGINE STATUS" /><div class="engine-grid"><div v-for="model in data.models.slice(0, 8)" :key="model.id"><i :class="model.status"></i><span>{{ model.label }}</span><strong>{{ statusLabel(model.status) }}</strong><small>{{ model.model }}</small></div></div></section>
        </div>

        <section class="screen-panel samples-panel"><PanelTitle title="典型风险样本实测" code="CURATED BENCHMARK · SANITIZED" /><RiskSampleStrip :samples="demoSamples" /></section>
      </main>
...
const riskOption = computed<EChartsCoreOption>(() => ({
  ...
  legend: { bottom: 0, textStyle: { color: '#7fa2b6', fontSize: 7 }, itemWidth: 7, itemHeight: 7, formatter: (name: string) => categoryNames[name] || name },
  series: [{ type: 'pie', radius: ['44%', '66%'], center: ['50%', '40%'], label: { show: false }, data: data.value?.risk_distribution.length ? data.value.risk_distribution : [{ name: '无风险事件', value: 1, itemStyle: { color: '#244d68' } }] }],
}))
