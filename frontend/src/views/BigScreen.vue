...
const trendOption = computed<EChartsCoreOption>(() => ({
  ...
  xAxis: { type: 'category', boundaryGap: false, data: data.value?.timeline.map(item => new Date(item.start).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false })) || [], axisLine: { lineStyle: { color: '#285e7a' } }, axisLabel: { color: '#91b6c9', fontSize: 9 } },
  yAxis: { type: 'value', minInterval: 1, axisLabel: { color: '#91b6c9', fontSize: 9 }, splitLine: { lineStyle: { color: 'rgba(80,168,208,.19)' } } },
  series: [
...
const riskOption = computed<EChartsCoreOption>(() => ({
  ...
  legend: { type: 'scroll', orient: 'vertical', right: 8, top: 'middle', height: '82%', width: '50%', itemGap: 7, selectedMode: false, pageIconSize: 7, pageTextStyle: { color: '#91b6c9', fontSize: 8 }, textStyle: { color: '#b0d2df', fontSize: 9, overflow: 'truncate', width: 108 }, itemWidth: 8, itemHeight: 8, formatter: (name: string) => categoryNames[name] || name },
...
.big-screen{--ink:#edf8ff;--muted:#9bb9c9;--blue:#23c6ff;--mint:#54e7b8;--amber:#f4b84f;--danger:#ff5d74;font-family:"HarmonyOS Sans SC","Noto Sans SC","Source Han Sans SC","Microsoft YaHei",sans-serif;background-color:#031222;background-image:radial-gradient(ellipse at 50% 40%,rgba(17,104,171,.22),transparent 42%),linear-gradient(rgba(50,130,172,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(50,130,172,.045) 1px,transparent 1px);background-size:auto,52px 52px,52px 52px}
.screen-header{height:68px;border-bottom-color:rgba(58,143,190,.5)}.screen-header::after{left:31%;right:31%;height:3px;background:var(--blue);box-shadow:0 0 17px rgba(35,198,255,.7)}.brand-lockup{gap:12px}.brand-lockup strong{font-size:16px;font-weight:700;color:var(--blue)}.brand-lockup span{font-size:9px;color:#79a7be}.screen-title{font-size:29px;font-weight:650;color:var(--ink)}.screen-clock strong{font-size:23px;color:#c7f5ff}.screen-clock span{font-size:10px;color:var(--muted)}
.screen-kpis{height:88px;gap:10px;padding:11px 0}.screen-kpis>div{padding:9px 13px;background:linear-gradient(132deg,rgba(10,52,78,.94),rgba(4,23,39,.96) 76%);border-color:rgba(51,142,192,.76);border-left-width:4px}.screen-kpis span{font-size:11px;color:#b7d2df}.screen-kpis strong{font-size:30px;font-weight:600;color:#f3fbff}.screen-kpis small{font-size:8px;color:#74a0b5}.screen-kpis .risk{border-left-color:var(--danger)}.screen-kpis .risk strong{color:#ff7186}
